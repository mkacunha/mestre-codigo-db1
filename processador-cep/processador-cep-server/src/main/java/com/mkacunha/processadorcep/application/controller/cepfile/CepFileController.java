...
public class CepFileController {
    ...
    public ResponseEntity upload(@RequestHeader("session") String session, @RequestParam("file") MultipartFile file) {
        try {
            CepFile cepFile = cepFileTranslator.apply(session, file);
